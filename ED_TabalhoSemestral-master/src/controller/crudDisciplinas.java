...
public class crudDisciplinas implements ActionListener {
	...
	private void atualizaDisciplina() throws Exception {
		...
		if (auxDisciplina.getDiaSemanaDisciplina() != null) {
	        ...
	        Lista<String> lista = alimentaLista();
	        Lista<String> camposAlterados = new Lista<>();
	        String idBusca = auxDisciplina.getIdDisciplina();

	        for (int i = 0; i < lista.size(); i++) {
	            String[] vetor = lista.get(i).split(";");

	            if (vetor[0].equals(idBusca)) {

	                String novoNome = tfDisciplinaNome.getText();
	                String novoDia = tfDisciplinaDia.getText();
	                String novoHorario = tfDisciplinaHora.getText();
	                String novaQtdHoras = tfDisciplinaQuantidadeHora.getText();
	                String novoCodigoCurso = tfDisciplinaCurso.getText();

	                if (!vetor[1].equals(novoNome)) camposAlterados.add("nome", i);
	                if (!vetor[2].equals(novoDia)) camposAlterados.add("dia da semana", i);
	                if (!vetor[3].equals(novoHorario)) camposAlterados.add("horário", i);
	                if (!vetor[4].equals(novaQtdHoras)) camposAlterados.add("quantidade de horas", i);
	                if (!vetor[5].equals(novoCodigoCurso)) camposAlterados.add("código do curso", i);
	              
	                String novaLinha = String.join(";", idBusca, novoNome, novoDia, novoHorario, novaQtdHoras, novoCodigoCurso);
	                lista.add(novaLinha, i);
	                break;
	            }
	        }
	     
	        try (BufferedWriter bw = new BufferedWriter(new FileWriter("disciplinas.csv"))) {
	            for (int i = 0; i < lista.size(); i++) {
	        ...
	        if (!camposAlterados.isEmpty()) {
	            taDisciplinaLista.setText("Disciplina atualizada");
	        } else {
	            taDisciplinaLista.setText("Nenhuma mudança detectada.");
	        }
